...
var gridCaption = 'รายการจัดพิมพ์บัตรคุมเงินสงเคราะห์';
var gridColNames = ['', 'หน่วยต้นสังกัด', 'เลขทะเบียนสมาชิก', 'เลขประจำตัวประชาชน', 'ยส - คำนำหน้า', 'ชื่อ', 'สกุล', 'ประเภทการชำระเงิน', 'เริ่มชำระศพที่', ''];
var gridColModel = [
    ...
    {name: 'paymentTypeCode', index: 'm.payment_type_code', align: 'center', sortable: true, width: 140},
    ...
    $(gridName).jqGrid({
        ...
        loadComplete: function() {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if ('10' === $(gridName).jqGrid('getCell', ids[i], 'paymentTypeCode')) {
                    paymentTypeCode = 'ชำระผ่านหน่วย';
                } else if ('11' === $(gridName).jqGrid('getCell', ids[i], 'paymentTypeCode')) {
                    paymentTypeCode = 'ผู้ชำระแทน';
                } else if ('20' === $(gridName).jqGrid('getCell', ids[i], 'paymentTypeCode')) {
                    paymentTypeCode = 'ชำระด้วยตนเอง';
                } else if ('21' === $(gridName).jqGrid('getCell', ids[i], 'paymentTypeCode')) {
                    paymentTypeCode = 'ชำระผ่านธนาณัติ';
                } else if ('22' === $(gridName).jqGrid('getCell', ids[i], 'paymentTypeCode')) {
                    paymentTypeCode = 'ชำระผ่านธนาคาร';
                } else {
                    paymentTypeCode = '';
                }
                $(gridName).setRowData(ids[i], {action: buttonView, paymentTypeCode: paymentTypeCode});
            }
            enableTooltips(this);
            //styleCheckbox(this);
